            ...
            {items.map((item, idx) => (
                ...
                >
                    <AnimatePresence>
                        {hoveredIndex === idx && (
                            <motion.div
                                className="absolute inset-0 h-full w-full bg-neutral-200 dark:bg-purple/[0.05] block rounded-3xl "
...
export const Card = ({
    ...
    return (
        <div
            className={cn(
                "flex rounded-2xl h-full w-full p-3 overflow-hidden dark:bg-glass-gradient border border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-50 shadow-[0px_5px_50px_0px_rgba(165,_39,_255,_0.30)] ",
                className,
            )}
        >
